...
questions_list <- intro %>%
  group_by(Numero, Questions, Style) %>%
...
for (i in seq_len(nrow(questions_list))) {
  ...
  cat("<div class='question-block' style='margin:20px 0; padding:15px; margin-left:4%; margin-right:4%;'>\n")
  ...
  if (style == "radio") {
    for (j in seq_along(reponses)) {
      ...
      cat(sprintf(
        "  <input type='radio' id='%s' name='q%s' value='%s'>
           <label for='%s' style='display:inline-block; margin:4px 15px 4px 0; border-color:rgba(239,119,87,1) !important; color: #293574; font-weight: bold; opacity: 0.9;'>%s</label>\n",
    ...
    cat(sprintf(
      "  <select name='q%s' style='width:100%%; margin-top:6px; border-color:rgba(239,119,87,1) !important; border-radius:6px; height:35px; color: #293574; font-weight: bold; opacity: 0.9;'>\n",
      numero
    ))
    for (r in reponses) {
