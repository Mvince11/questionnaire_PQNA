...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Bloc question 8a (syndicat mixte)
  const q8a = document.querySelector('#q8a');
  if (q8a) {
    if (valIntro === "un syndicat mixte") {
      q8a.style.display = "block";
      q8a.classList.add("visible");
    } else {
      q8a.style.display = "none";
      q8a.classList.remove("visible");
    }
  }
...
<button style="background-color: #ef7757;
  ...
  font-family: normal;
  border-color: coral;"
  onclick="window.location.href='dynamique_transition_territoriale.html';"> < Précédent</button>
...
{{< include _includes/custom_footer.html >}}
 ...
 <button style="background-color: #ef7757;
  ...
  font-family: normal;
  border-color: coral;"
  onclick="window.location.href='territoire_intelligent.html';"> Suivant > </button>
